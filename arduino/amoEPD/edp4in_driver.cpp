...
void EPD4INC::EPD_SetFount( FONT font_index)
{
  ...
  switch (font_index)
  {
    ...
    case 1: Epd_Font.Fontname_En = "/fonten14.bin";    Epd_Font.Fontname_Cn = "\0";              f = SPIFFS.open(Epd_Font.Fontname_En, "r");  Epd_Font.cn_px = 14; Epd_Font.cn_py = 14; Epd_Font.en_px = 7; Epd_Font.en_py = 14; Epd_Font.part_dx = 1; Epd_Font.Font_Index = 1; break;
...
uint16_t EPD4INC::DrawUnicodeChar(int16_t x, int16_t y,  unsigned char *code, unsigned char isinv)
{
  ...
  // Serial.printf("\n DrawUnicodeChar charcode is%d\n",charcode);   //字符

  //  if (charcode > 0x80) //非ASCII字符  按照汉字计算  注意  不存在的字库需要处理  暂未处理
  // {
  ...
  //   Serial.printf("\n DrawUnicodeChar charcode offset is%d\n",offset);   //字符
  // }
  //  else    //ASCII字符
  //  {
  //    epd_area.height = Epd_Font.en_py;
  //    epd_area.width =  Epd_Font.en_px;
  //    sizeofsinglechar = Epd_Font.en_py * Epd_Font.en_px / 8; //48 24 /2
  //    offset = charcode * sizeofsinglechar;
  //    fontname = Epd_Font.Fontname_En;
  //  //  Serial.printf("\n DrawUnicodeChar is EN   n");   //字符
  //  }
  //  Serial.printf("\n DrawUnicodeChar fontname  is%s  ofset is %d\n",fontname,offset);   //字符
  char fontbuf[sizeofsinglechar];
  //  Serial.print("\n DrawUnicodeChar load font data   star \n");
...
int EPD4INC::User_Font_Tran(uint16_t fontsize, const  uint8_t* p_img, uint16_t mode, const struct pl_area *area, uint8_t isinv)
{
  ...
  else if (Epd_Font.Font_Index == 1) //14#
  {

    charbuf[0] = p_img[0];
    charbuf[1] = (p_img[1] & 0x3F) + ((p_img[2] << 6) & 0xc0);
    charbuf[2] = ((p_img[2] >> 2) & 0x3F) + ((p_img[3] << 6) & 0xC0);
    charbuf[3] = ((p_img[3] >> 2) & 0x0F) + ((p_img[4] << 4) & 0xF0);
    charbuf[4] = ((p_img[4] >> 4) & 0x0F) + ((p_img[5] << 4) & 0xF0);
    charbuf[5] = ((p_img[5] >> 4) & 0x03) + ((p_img[6] << 2) & 0xFC);
    charbuf[6] = ((p_img[6] >> 6) & 0x03) + ((p_img[7] << 2) & 0xFC);
    p_img = p_img + 8;
    charbuf[7] = p_img[0];
    charbuf[8] = (p_img[1] & 0x3F) + ((p_img[2] << 6) & 0xc0);
    charbuf[9] = ((p_img[2] >> 2) & 0x3F) + ((p_img[3] << 6) & 0xC0);
    charbuf[10] = ((p_img[3] >> 2) & 0x0F) + ((p_img[4] << 4) & 0xF0);
    charbuf[11] = ((p_img[4] >> 4) & 0x0F) + ((p_img[5] << 4) & 0xF0);
    charbuf[12] = ((p_img[5] >> 4) & 0x03) + ((p_img[6] << 2) & 0xFC);
    charbuf[13] = ((p_img[6] >> 6) & 0x03) + ((p_img[7] << 2) & 0xFC);
    p_img = p_img + 8;
    charbuf[14] = p_img[0];
    charbuf[15] = (p_img[1] & 0x3F) + ((p_img[2] << 6) & 0xc0);
    charbuf[16] = ((p_img[2] >> 2) & 0x3F) + ((p_img[3] << 6) & 0xC0);
    charbuf[17] = ((p_img[3] >> 2) & 0x0F) + ((p_img[4] << 4) & 0xF0);
    charbuf[18] = ((p_img[4] >> 4) & 0x0F) + ((p_img[5] << 4) & 0xF0);
    charbuf[19] = ((p_img[5] >> 4) & 0x03) + ((p_img[6] << 2) & 0xFC);
    charbuf[20] = ((p_img[6] >> 6) & 0x03) + ((p_img[7] << 2) & 0xFC);
    p_img = p_img + 8;
    charbuf[21] = p_img[0];
    charbuf[22] = (p_img[1] & 0x3F) + ((p_img[2] << 6) & 0xc0);
    charbuf[23] = ((p_img[2] >> 2) & 0x3F) + ((p_img[3] << 6) & 0xC0);
    charbuf[24] = ((p_img[3] >> 2) & 0x0F) + ((p_img[4] << 4) & 0xF0);
    charbuf[25] = ((p_img[4] >> 4) & 0x0F) + ((p_img[5] << 4) & 0xF0);
